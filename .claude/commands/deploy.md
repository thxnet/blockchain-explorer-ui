...
3. Build: `docker build --platform linux/amd64 -t ghcr.io/thxnet/blockchain-explorer-ui:build-local-$(date +%Y.%m.%d)-$(git rev-parse --short HEAD) .`
4. Push: `docker push ghcr.io/thxnet/blockchain-explorer-ui:build-local-$(date +%Y.%m.%d)-$(git rev-parse --short HEAD)`
